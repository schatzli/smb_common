...
SmbController::SmbController(ros::NodeHandle &nh)
    : nh_(nh), mmInterface_(), buffer_(), tfListener_(buffer_) {
  initialize();
}
...
bool SmbController::initialize() {
  ...
  rosPublishingWorker_ = nh_.createTimer(
      ...
      [this](const auto &) { this->publishRos(); });

  // Start mpc update worker.
  mpcUpdateWorker_ = nh_.createTimer(
      ros::Duration(1 / mmInterface_.mpcSettings().mpcDesiredFrequency_),
      [this](const auto &) { this->mpcUpdate(); });

  trackingControllerWorker_ = nh_.createTimer(
      ros::Duration(1 / mmInterface_.mpcSettings().mrtDesiredFrequency_),
      [this](const auto &) { this->advance(); });
...
bool SmbController::advance() {
  ...
  if (planAvailable_) {
    ocs2::vector_t controlInput(
        (int)smb_path_following::SmbDefinitions::INPUT_DIM);
    ocs2::vector_t optimalState(
        (int)smb_path_following::SmbDefinitions::STATE_DIM);
    size_t subsystem;
...
bool SmbController::mpcUpdate() {
  if (!observationAvailable_) {
    ROS_INFO_STREAM_THROTTLE(3, "[SmbController::mpcUpdate] observation "
...
void SmbController::writeDesiredTrajectory(
  ...
  if (path.poses.size() == 0) {
    costDesiredTrajectories.desiredStateTrajectory().push_back(
        currentObservation.state);
    costDesiredTrajectories.desiredTimeTrajectory().push_back(
        currentObservation.time);
    costDesiredTrajectories.desiredInputTrajectory().push_back(
    ...
    for (int i = 0; i < path.poses.size(); ++i) {
      ocs2::vector_t targetState(smb_path_following::SmbDefinitions::STATE_DIM);
      ...
      costDesiredTrajectories.desiredInputTrajectory().push_back(
          ocs2::vector_t::Zero(smb_path_following::SmbDefinitions::INPUT_DIM));
      costDesiredTrajectories.desiredTimeTrajectory().push_back(
...
void SmbController::resetMpc() {
  observation_ = Observation();
  observation_.state = SmbDefinitions::defaultState();
  observation_.input.setZero(smb_path_following::SmbDefinitions::INPUT_DIM);
